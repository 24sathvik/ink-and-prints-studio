...
export function KanbanCard({ 
  ...
  return (
    <div
      ...
      className={`bg-white p-3 rounded-lg border shadow-sm relative group cursor-grab active:cursor-grabbing flex flex-col gap-2 ${isOverlay ? 'shadow-xl scale-105 rotate-2' : ''}`}
    >
      ...
      <div className="flex justify-between items-end mt-2 pt-2 border-t">
        ...
        <div className="flex items-center gap-2">
          {card.invoice?.id && (
            <Link 
              href={`/invoices/${card.invoice.id}/edit`} 
          ...
          <div 
            className="w-6 h-6 rounded-full bg-slate-200 text-slate-700 flex items-center justify-center text-[9px] font-bold" 
